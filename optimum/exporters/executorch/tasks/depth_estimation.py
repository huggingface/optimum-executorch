...
@register_task("depth-estimation")
def load_depth_estimation_model(model_name_or_path: str, **kwargs) -> VisionEncoderExportableModule:
    """
    Loads a vision model for depth estimation and registers it under the task
    'depth-estimation' using Hugging Face's `AutoModelForDepthEstimation`.

    Args:
        model_name_or_path (str):
            Model ID on huggingface.co or path on disk to the model repository to export. For example:
            `model_name_or_path="depth-anything/Depth-Anything-V2-Small-hf"` or `mode_name_or_path="/path/to/model_folder`
        **kwargs:
    ...
    eager_model = AutoModelForDepthEstimation.from_pretrained(model_name_or_path, **kwargs).to("cpu").eval()
